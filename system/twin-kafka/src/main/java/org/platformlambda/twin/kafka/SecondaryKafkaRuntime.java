...
public final class SecondaryKafkaRuntime {
    private static final Logger log = LoggerFactory.getLogger(SecondaryKafkaRuntime.class);
     ...
     * expiry). Closing the producer afterwards flushes any buffered records. The twin of
     * {@code KafkaRuntime.shutdown()} on the primary cluster.
     * <p>
        ...
        try {
            ...
            if (runningAdapter != null) {
                try {
                    ...
                    log.info("Secondary Kafka flow adapter closed - its consumers have left their groups");
                } catch (RuntimeException e) {
                    log.warn("Secondary Kafka flow adapter did not close cleanly - {}", e.toString());
                }
            }
