...
```bash
node scripts/playground-session-broker.mjs --target http://127.0.0.1:8305 --port 8765
```

Both flags are runtime overrides: `--target` points the broker at whatever host and port the
app serves — this example's default is `8305` (`rest.server.port`), while the broker's built-in
...
The graph to co-author here is **`profile-cache`** — bring it into a session with
`import graph from profile-cache`, edit it, dry-run it with `run`, then `export graph as
profile-cache` and restart the app to redeploy it through the CompileGraph gate.
