...
**Nothing for Scenario 9.** The token-bearing re-drive ran with a mock provider (drive 9 above) and exported from
all four applications; the maintainer's look at those four traces in Dynatrace is a formality — Scenario 9's
backend view is CONFIRMED on the drive-8 traces (2026-09-22, the maintainer's four screenshots: one tree per
trace, the edge span the root and the response time), and so is Scenario 8's — the broken tree shape it revealed
is what Scenario 9 fixed. A real-provider token stream of the same shape follows whenever quota returns; nothing
