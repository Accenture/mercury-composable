...
build_log:
  ...
  deviations_from_design:
    ...
    - id: "D-B3"
      ...
      reason: "Resolves OQ-D02 empirically (probed in-session). The engine's substituteVarIfAny (GraphLambdaFunction) leaves any { } whose key contains ':' / newline / tab as-is and SWALLOWS {model.*} refs nested inside such a brace; multi-line ''' COMPUTE values also fail to execute. The single-line top-level-var form keeps every {model.*} ref outside all braces. Same algorithm and responsibility (FLOW-003/004/005/006, FAIL-004/005, OUT-005)."
