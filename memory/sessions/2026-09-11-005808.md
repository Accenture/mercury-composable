...
- Invariants:    DUE (45 ≥ 40) — prompt raised as ot-reverify-invariants-20260911, then
                 CONFIRMED same-day by Eric: all 18 core facts + the Vision hold
                 (walkthrough with live-tree evidence; no supersessions).
                 stack-build-maven reworded per his ruling (the reactor stays Maven;
                 consumer templates ship Maven-or-Gradle) and thread-add-gradle-build
                 declared COMPLETE (consumer-side Gradle delivered by the templates);
                 both threads closed as stubs awaiting the sweep window
...
- ot-reverify-invariants-20260911 (created, then closed same-day — Eric confirmed
  all 18 core facts + the Vision)
- stack-build-maven (updated — reworded at the confirmed re-verify: reactor stays
  Maven; consumer templates ship Maven-or-Gradle)
- thread-add-gradle-build (closed — Eric's ruling: complete via the starter
  templates' consumer-side Gradle option)
