...
cause's message. The status is now resolved from the **cause chain** (`Utility.getStatusFromException`: the first `AppException`, `TimeoutException` → 408 or `IllegalArgumentException` → 400 wins; 500 only when none is present), one rule shared by both mappers and by `EventStreamWriter.fail`, with the documented table in the Event Envelope reference updated to match (PR #427). The full drive was then run again against both mains — the
...
server answered (a wrong type, an unknown command) on the default 500. The classifier lives in the shared `redis-connection` foundation (`RedisFailure`) and is applied by `RedisCache`; the Rust engine classifies in its foundation's command path, so the two engines fail alike. The
