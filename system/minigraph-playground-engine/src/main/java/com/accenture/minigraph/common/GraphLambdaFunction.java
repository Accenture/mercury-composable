...
public abstract class GraphLambdaFunction implements TypedLambdaFunction<EventEnvelope, Object> {
    ...
    protected List<String> combine(String route, String nodeName, MiniGraph graph, List<String> entries) {
        List<String> merged = new ArrayList<>();
        for (var entry : entries) {
            var line = entry.trim().toLowerCase();
            if (line.startsWith(EXECUTE_TAG)) {
                mergeStatements(route, nodeName, line.substring(EXECUTE_TAG.length()).trim(), graph, merged);
            } else {
                merged.add(line);
            }
        }
    ...
    protected List<List<String>> splitBlocks(List<String> statements) {
        ...
        var hasBegin = false;
        for (var entry : statements) {
            if (BEGIN.equalsIgnoreCase(entry.trim())) {
                hasBegin = true;
                break;
            }
        }
        if (hasBegin) {
            distributeBlocks(statements, result);
        } else {
            // when there is no begin statement, set the whole block as the 2nd one so the for-each will execute it.
            second.addAll(statements);
        }
        return result;
    }

    private void distributeBlocks(List<String> statements, List<List<String>> blocks) {
        var first = blocks.getFirst();
        var second = blocks.get(1);
        var third = blocks.getLast();
        var n = 1;
        for (var entry : statements) {
            var text = entry.trim();
            if (n == 1) {
