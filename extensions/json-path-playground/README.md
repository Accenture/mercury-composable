...
5. Enter "help" in command input box for more instruction
6. Enter "load" to load the JSON/XML text and render it into a JSON object
7. The JSON object will be populated in a "result" node.
8. Enter "$.result" to try out the first JSON-Path command
9. Enter "result" to try out simple retrieval command
...
Entering `result.hello` will return "world".
