...
- **latest_release:** v4.12.15 (2026-09-23 01:35:26Z — **the connected-trace release, a lock-step round on all four
  ...
  **Lockstep (one number on all four, [[conv-ports-adopt-java-release-number]]):** the Rust port released v4.12.15 the same
  ...
  on node, the `llm.chat`/`llm.stream` AI nodes. Registry publications the same night: crates.io 12 of 12 crates by 01:57Z (one `cargo publish --workspace`, no retry), npm
  4.12.15 at 02:34Z (`latest`; a first attempt failed 404 on an expired token — npm answers an unauthenticated PUT with 404,
  `npm login` fixed it), PyPI 4.12.15 at 02:34Z (wheel + constrained sdist). Next: the AI SDLC/MCP backlog
