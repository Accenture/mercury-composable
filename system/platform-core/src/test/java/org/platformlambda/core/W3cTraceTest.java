...
class W3cTraceTest {
    ...
    void buildsValidTraceparent() {
        ...
        assertEquals("00-" + TRACE_ID + "-" + SPAN_ID + "-01",
                W3cTrace.format(TRACE_ID, SPAN_ID));
    }

    @Test
    void roundTripsThroughParse() {
        var value = W3cTrace.format(TRACE_ID, SPAN_ID);
    ...
    void mercuryGeneratedIdsAreW3cCompatible() {
        ...
        var value = W3cTrace.format(traceId, spanId);
    ...
    void rejectsNonW3cIds() {
        ...
        assertNull(W3cTrace.format("ch-3", SPAN_ID));
        assertNull(W3cTrace.format(TRACE_ID, "short"));
        assertNull(W3cTrace.format(null, SPAN_ID));
        assertNull(W3cTrace.format(TRACE_ID, null));
        // all-zero ids are invalid per the W3C spec
        assertNull(W3cTrace.format("0".repeat(32), SPAN_ID));
        assertNull(W3cTrace.format(TRACE_ID, "0".repeat(16)));
    }

    @Test
    void parseRejectsMalformedValues() {
        assertEquals(0, W3cTrace.parse(null).length);
        assertEquals(0, W3cTrace.parse("").length);
        assertEquals(0, W3cTrace.parse("not-a-traceparent").length);
        assertEquals(0, W3cTrace.parse("00-" + TRACE_ID + "-" + SPAN_ID).length); // missing flags field
        assertEquals(0, W3cTrace.parse("00-tooShortTraceId-" + SPAN_ID + "-01").length);
        assertEquals(0, W3cTrace.parse("00-" + TRACE_ID + "-0000000000000000-01").length); // zero parent id
    }
