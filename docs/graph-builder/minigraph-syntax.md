...
{constant} -> model.{key}
```
Constants: `text(value)`, `int(n)`, `long(n)`, `float(n.n)`, `double(n.n)`, `boolean(true|false)` (also `map(...)`, `file(...)`, `classpath(...)`). **There is no `number()`** — use `double(n.n)` or `float(n.n)` for decimals. An unrecognized constant makes the whole `instantiate` fail silently (the instance is dropped and every `inspect` returns `404`).
...
- `model.none` — null initializer (use before array append)
...
- Named target = override traversal to that node
- Operators: `==`, `!=`, `>`, `<`, `>=`, `<=`, `&&`, `||`
- `!= null` — direct null check (graph.math)
...
output[]=f:defaultValue(fetcher-config.result.field, model.zero) -> model.config.field
```
