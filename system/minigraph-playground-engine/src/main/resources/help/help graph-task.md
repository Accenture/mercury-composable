...
The source of an output entry is a constant, 'result' or 'result.{key}', 'model.{key}' or this node's
own namespace - 'input.*' is valid only on the input side. To echo a request value, stage it at a
