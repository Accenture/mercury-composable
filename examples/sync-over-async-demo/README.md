...
### Terminal B — Kafka
```shell
cd helpers/kafka-standalone && java -jar target/kafka-standalone-x.y.z-exec.jar
```
...
```shell
java -Dspring.profiles.active=backend -jar examples/sync-over-async-demo/target/sync-over-async-demo-x.y.z.jar
```
### Terminal D — facade pod (REST on :8400)
```shell
java -Dspring.profiles.active=facade -jar examples/sync-over-async-demo/target/sync-over-async-demo-x.y.z.jar
```
...
- **Multiple facade pods on one machine** (requires the multi-partition topics created above). Start a
  ...
  ```shell
  java -Dspring.profiles.active=facade -Drest.server.port=8401 -jar examples/sync-over-async-demo/target/sync-over-async-demo-x.y.z.jar
