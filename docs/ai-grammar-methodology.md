...
chains. Java virtual threads later made sequential programming practical over the event-driven
core, allowing the architecture to retain its decoupling without forcing application developers
into a reactive style.
...
exhaustive map with a CI coverage gate — a new guide cannot ship unlisted. The Rust engine uses a
...
The Java engine is the reference implementation, and the Rust engine tracks the same contracts.
Flow YAML can move across the two engines. Python and Node.js functions can participate as
Event-over-HTTP peers using the same envelope.
...
guides, catalogs, and validation rules.
...
cautionary tale. Twin-kafka is a building block built on a building block — a second Kafka
cluster on top of the minimalist Kafka library. While its entry was missing from the discovery
map, the module was effectively invisible to AI partners, who fell back to reading source. The
entry landed, with a CI gate behind it, and one hop of discovery replaced the hunt.
...
a link-integrity gate on the Rust engine's curated map.
...
Development" (Eric Law, August 2026) and "The Mercury Story — Origin, Evolution, and the AI
Grammar Methodology" (this repository, September 2026), whose Mercury narrative it carries
forward. Sources: this repository's `docs/guides/` and `docs/arch-decisions/ADR.md`; the shared
memory layer (`memory/vision.md`, `memory/continuity.md`); records of the ai-enabled-repo-demo
exercises; measurements taken 2026-09-04 on the Rust engine's documentation map. All figures are
