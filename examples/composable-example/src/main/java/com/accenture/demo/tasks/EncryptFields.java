...
public class EncryptFields implements TypedLambdaFunction<Map<String, Object>, Map<String, Object>> {
        ...
        if (input.containsKey(DATASET)) {
            ...
            for (String f: fields) {
                if (map.exists(f)) {
                    ...
                    map.setElement(f, encryptField(clearText));
                }
            }
            return dataset;
        } else {
    ...
    private String encryptField(String clearText) throws GeneralSecurityException, IOException {
        ...
        byte[] b = crypto.aesEncrypt(util.getUTF(clearText), key);
