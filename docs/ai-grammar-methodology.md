---
title: The Mercury Story
summary: White paper — why Mercury exists, its ascent from code to configuration to knowledge,
  and the AI grammar methodology for building applications and building blocks with an AI partner.
audience: [developer, architect, business, ai-agent]
keywords: [ai grammar, composable, active knowledge graph, human-ai collaboration, methodology, white paper]
---
...
from those runs shipped to the field in v4.12.2.*
...
The founding decision came from the actor-model lineage (Scala/Akka) carried onto the Eclipse
Vert.x event bus: **functions know nothing about each other.** A Mercury function is a
...
scale. The industry's answer so far is more review. Mercury's answer is different: **change the
...
every team. A platform meant to be built *on* by AI partners has to make itself legible at a
price that scales.
...
| Three DSL spec kits | For each authoring surface — `rest.yaml`, Event Script flows, MiniGraph commands — a grammar reference, a machine-readable JSON catalog, an AI agent guide, and a **CI drift test** binding them to the engine. |
...
a hundred increments later it ships in lock-step with the Java engine, with the same discipline
intact.
...
- **Recommended for user applications: Layer 3, the knowledge graph.** Express the service as a
...
framework's own Kafka adapter family — is implemented with **layer-2 and layer-3 patterns**:
...
**The recursion is already live inside the framework.** Twin-kafka is a building block built on
a building block — a second Kafka cluster on top of minimalist-kafka — and this week supplied
...
- **`llms.txt`** — the emerging convention for machine-readable site maps. Mercury adopts it and
...
- **Docs-as-code** — documentation versioned, reviewed, and built like software. Mercury extends
  it to *docs-as-contract*: drift tests fail the build when a guide and its engine disagree.
- **Architecture Decision Records** (Nygard) — Mercury's ADR ledger holds the durable rationale;
  the memory layer's facts point at ADRs, so agents inherit the *why*, not just the what.
- **Consumer-driven contract testing** — the golden-vector suites (envelope wire format,
  registration metadata) are contract tests shared verbatim between independent engine
  implementations, the same trust mechanism Pact-style testing brought to service boundaries.
- **Agent-instruction conventions** (`AGENTS.md` and kin) — Mercury layers a routing shim on
...
- **Event-driven architecture and the actor model** — the substrate itself is orthodox EDA;
...
- **Human-in-the-loop governance** — dry-run before deploy, compile gates, human certification,
  ...
  delivery pipelines; the graph lifecycle applies them to model artifacts.
- **Evaluation culture** — the demo's fresh-agent rehearsals are evals for documentation; the
  memory smoke test is an eval for project memory. Both run on cadence, both produced fixes.
