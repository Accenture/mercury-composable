...
The phase outputs a **graph design specification** — node responsibilities, skills, edges, mappings, state paths, control-flow and error/fallback choices, and the inspection points `/test` will use — structured per the [Graph Design Specification Template](#graph-design-specification-template) below, which is the canonical schema for what it must contain (including `requirement_traceability` mapping every requirement to its design elements).
...
> **Runnable exemplars (start here):** [patterns/README.md](./patterns/README.md) maps each intent below to a **verified, runnable** graph. Six in-repo exemplars are `✓live` — built green on the engine and ledger-backed (linear, fetch-and-shape, fan-out-join, decision, iterative, extension) — plus the one pattern the tutorials don't cover, [per-item failure isolation](./patterns/per-item-isolation.md). **Compose by copying the nearest exemplar** (its build/runtime constraints travel with it); the primitive table above is for *selecting* the shape, not assembling it from scratch.
