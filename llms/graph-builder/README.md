...
The MiniGraph engine brands these graphs **Active Knowledge Graphs** — see the engine's `system/minigraph-playground-engine/README.md` and its tutorial 1. This workflow teaches the *same* model in operational terms, and so does the engine's own help (tutorial 1: an Active Knowledge Graph is *"a 'property graph' that contains one or more 'active' nodes; an active node is associated with a 'skill' backed by a composable function"*). Use this bridge to map the engine's branded vocabulary to the operational terms used throughout these docs:

| Engine (brand) vocabulary | This workflow's operational term | What it actually is |
|---|---|---|
| Active Knowledge Graph | the graph you author / deploy | a `root`, an `end`, and skill / no-skill nodes wired by edges |
| Active node | **skill node** | a node whose `skill` property drives behavior |
| Data entity node | **no-skill node** or **dictionary node** | a node holding data attributes with no `skill` (see [Data Dictionary Pattern](./minigraph-syntax.md#data-dictionary-pattern)) |
| Attribute on an entity | node-local state under the node alias | see [Node Property Initialization](./graph-design.md#node-property-initialization) |

There is **no separate "entity" primitive in the engine runtime** — a "data entity" is just a no-skill or dictionary node (or a `graph.island` when it must be reachable but must not execute downstream). The branding does not change any mechanics below; it only names the same model differently. When a user arrives thinking in "Active Knowledge Graph / entity" terms, translate through this table and proceed with the operational vocabulary.
...
| [briefs/hello-world-requirements-brief.md](./briefs/hello-world-requirements-brief.md) | Example | A filled, gate-passed worked example; its `/design` and `/build` counterparts are [hello-world-design-spec.md](./briefs/hello-world-design-spec.md) and [hello-world-build-log.md](./briefs/hello-world-build-log.md) in the same folder. |
