...
public class GraphJs extends GraphLambdaFunction {
    ...
    private String executeNode(Context context, String nodeName, GraphInstance graphInstance,
        ...
        if (forEach.isEmpty()) {
            return executeStatements(context, nodeName, merged, graphInstance);
        } else {
            List<List<String>> blocks = splitBlocks(merged);
            var result1 = executeStatements(context, nodeName, blocks.getFirst(), graphInstance);
            if (!NEXT.equals(result1)) {
                return result1;
            }
            var result2 = executeForEach(context, nodeName, forEach, blocks.get(1), graphInstance);
            if (!NEXT.equals(result2)) {
                return result2;
            }
            return executeStatements(context, nodeName, blocks.getLast(), graphInstance);
        }
    }

    private String executeForEach(Context context, String nodeName, List<String> forEach, List<String> statements,
        ...
        if (!statements.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                var result = executeStatements(context, nodeName, statements, graphInstance);
