...
public class HstsFilter implements Filter {
    ...
    private static final AtomicBoolean HSTS_REQUIRED = new AtomicBoolean(false);
    ...
    public void init(FilterConfig filterConfig) {
        if (!LOADED.get()) {
            ...
            HSTS_REQUIRED.set("true".equals(reader.getProperty("hsts.feature", "true")));
            log.info("HSTS (RFC-6797) feature {}", HSTS_REQUIRED.get()? "enabled" : "disabled");
        }
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (request instanceof HttpServletRequest req && response instanceof HttpServletResponse res) {
            if (req.getHeader(UPGRADE) == null) {
                 ...
                 */
                if (HSTS_REQUIRED.get() && HTTPS.equals(req.getHeader(PROTOCOL))) {
