...
> **Build by example, not from rules.** The reliable path is to **copy the nearest verified, runnable exemplar** in [`patterns/`](./patterns/) — six shapes, each green-on-the-engine and ledger-backed: linear, fetch-and-shape, fan-out-join, decision, iterative, extension — then adapt it. This document is the **syntax reference** *behind* those exemplars: reach into a section when an exemplar leaves you unsure, not to assemble a graph from scratch. (You need an open session id first — see [Getting a session id](#getting-a-session-id--you-cannot-mint-one-over-rest).)

> **Verification legend:** claims tagged **(verified)** were execution-tested against the live engine and recorded in [`evidence/dogfood-customer-360.md`](./evidence/dogfood-customer-360.md). Unmarked syntax/behavior is documented from the engine's help surface and may not be execution-tested yet — treat it as **asserted** until dogfooded. A `(verified)` tag also **expires when the engine changes** — see [Verification Freshness](./evidence/dogfood-customer-360.md#verification-freshness).

---

## Read This First — Critical Footguns

These failures are **silent**: the POST returns `"accepted"`, nothing errors in the HTTP response, and the problem surfaces only on the WebSocket console or as a later `404`. Read this block before authoring; each item is detailed in full in its own section below.

**Golden rule — `"accepted"` is NOT success.** Every Companion POST returns `"accepted"` once *dispatched*, even for a malformed or no-op command. Command output streams to the WebSocket console, never the HTTP response. So **verify after every command** by re-fetching live state — see the two *MANDATORY: verify…* sections under [Companion API Execution](#companion-api-execution). The `companion.mjs` helper enforces this automatically; prefer it over hand-rolled `curl`.

The five highest-impact silent footguns:

1. **A bare `next` fans out to ALL natural outgoing edges.** A return of `next` (including `THEN:`/`ELSE: next` or a statement-less skill node) traverses *every* outgoing edge — so a node that returns bare `next` and must continue to one place needs exactly **one** natural outgoing edge. **This does NOT apply to an IF/THEN/ELSE that names its targets:** a named-target IF routes to exactly that branch even when the node *also* has natural edges to both branches (verified live, C-55 — `decide`→`dec-pass`+`dec-fail` natural edges, `85`→pass-only, `50`→fail-only). So a normal decision node may keep natural edges to its branches; only a bare-`next` return fans out. (See [Statement Syntax → IF/THEN/ELSE](#ifthenelse).)
2. **An `IF` halts the node silently when it references a value that does not resolve** — including a bare `{x} != null` when `x` is absent. There is no working bare null-check: detect null with the `:boolean(null=true)` mapper pattern, then branch on the resulting boolean. (See [Null Checking](#null-checking).)
3. **Wrap every multi-line value in `'''`.** An un-`'''`-wrapped IF/THEN/ELSE is *accepted* but makes the engine abort the whole property block — the node is created with **empty `properties`** (a hollow node). Re-fetch and confirm each `statement[]`/`mapping[]` actually applied. (See [create node](#create-node).)
4. **There is no `number()` constant** — use `int(n)`, `double(n.n)`, or `float(n.n)`. An unrecognized constant makes `instantiate` drop the *entire* instance silently; every later `inspect` then returns `404`. (See [instantiate graph](#instantiate-graph).)
5. **`inspect` serves containers only.** A scalar leaf (string/number/boolean) returns `404` **by design** even when the value exists — read the parent `Map`/`List`, not the dotted leaf. A `404` on a scalar path is *not* a halt. (See [run / inspect / seen / execute](#run--inspect--seen--execute).)

Also: never send a literal `...` line (→ `ERROR: Missing composite path`), and strip `\r` (CRLF→LF) before sending.

> **Ledger provenance** (these tags are a projection of the [claim ledger](./evidence/dogfood-customer-360.md#projection-index)): #1 → C-23/C-32/C-55, #2 → C-09/C-10, #3 → C-56, #4 → C-35, #5 → C-57 — all **verified** (the #3/#5 orphans were backed live 2026-06-16 at session `ws-177533-2`; C-55 clarifies #1's named-target vs bare-`next` distinction).
...
> **Engine vocabulary bridge:** the engine brands a graph an *Active Knowledge Graph*, a skill node an *active node*, and a no-skill/dictionary node a *data entity*. There is no distinct "entity" runtime primitive — see the [Conceptual Model](./README.md#conceptual-model) table. The enumeration below is the operational node-type ↔ skill list this workflow uses.
...
> All functions above are **execution-verified** — see [Appendix: Provenance and Sources](#appendix-provenance-and-sources).
...
- The right-hand side is evaluated as JavaScript (`graph.js`) after `{path}` substitution. **Multi-line COMPUTE works (verified)** — a COMPUTE may be written on one physical line or wrapped across several lines with `'''`; both reach the JS engine identically (the newline-flatten runs *after* compute — [why](#appendix-provenance-and-sources)). Complex IIFEs (nested object literals, `.filter`/`.reduce`) are ordinary JavaScript and evaluate normally.

#### `{path}` substitution — keep each ref in clean braces (verified)
A plain `{model.x}` resolves wherever it sits — inline, or nested inside an object literal or IIFE body — and an object literal such as `{tag: {model.x}}` keeps its outer braces as a JS object while the inner `{model.x}` still resolves. **Footgun:** a lone `{path}` that shares its braces with a colon/newline/tab/CR (e.g. `{model.x : 0}`) is left **verbatim**; `eval` then receives invalid JavaScript and the node **halts silently** — the same failure mode as any unresolved/bad `graph.js` expression. Keep each `{path}` ref in its own clean braces. (Source mechanism — innermost-first scan + the colon/newline guard — in the [Appendix](#appendix-provenance-and-sources).)
...
- **`RESET`** (source-verified) clears the named node(s) so a bounded loop can re-traverse them; always pair with an `IF` exit condition. The runtime enforces a tight-loop guard: a node hit too often aborts with `400 "executed too frequently"` (defaults: **10 hits / 1000ms**) — a backstop, not a substitute for the exit condition. (Source: [Appendix](#appendix-provenance-and-sources).)
...
> **Verification:** the verbs above are execution-verified except `RESET` and `BEGIN/END`, which are source-verified — see [Appendix: Provenance and Sources](#appendix-provenance-and-sources).
...
- **Target resolution (source-verified):** the target loads only from the read-only **deployed-graph location**, a *different* store from `export`/`import`. So `export graph as X` makes `X` importable but **not** callable as `extension=X` (→ `"not found"`, `400`). To make a graph extension-callable, deploy its JSON to the deployed location (a rebuild/restart) — the Companion API cannot deploy at runtime. (Config keys/paths: [Appendix](#appendix-provenance-and-sources).)
...
<example name="data-dictionary-pattern">
A complete provider→dictionary→fetcher cluster, annotated. The **verified, runnable** version of this shape is [patterns/fetch-and-shape.md](./patterns/fetch-and-shape.md) — copy that to build; the blocks below explain each part.
...
Composition design — choosing graph shape and the orchestrator / sub-graph / reusable-collection roles — belongs to [`graph-design.md`](./graph-design.md#design-patterns) and the runnable [`patterns/`](./patterns/) exemplars, not to this syntax reference. The one fact that *is* syntax: the engine defines no "graph types" — every graph is just a `root`, an `end`, and skill nodes wired together; mix building blocks freely.
...
### Getting a session id — you cannot mint one over REST

**This is the first thing a cold agent needs and the docs used to omit it.** The Companion endpoint **does not create sessions** — it dispatches into an **already-open** Playground WebSocket (it is *not* a headless runner; it's gated by `app.env=dev`). So:

1. A **Playground session must already be open** — the operator opens the Playground UI, or connects a client to `ws://<host>/ws/graph/playground`.
2. The first inbound WS frame carries the public id: `{ "type": "session", "id": "ws-123456-7" }` (format `ws-<6 digits>-<counter>`). That `{session-id}` is what every REST call (`POST /api/companion/{id}`, `GET /api/graph/session/{id}`, `GET /api/inspect/{id}/{key}`) uses.
3. **An agent driving over REST does not mint the id — the operator provides it.** You cannot create a session via `/api/companion`; if you don't have an id, ask for one. A `404 "No active session for id"` means the WebSocket/tab was closed or the id is wrong — get a fresh id, don't retry the same one.

Full flow + curl examples: [`webapp/docs/COMPANION_ENDPOINT_README.md`](../../system/minigraph-playground-engine/webapp/docs/COMPANION_ENDPOINT_README.md).
...
Provenance behind the `(verified)` / `(source-verified)` tags above. **You do not need this to build a graph** — it is the audit trail for verifying or maintaining the rules. Full execution evidence (session ids + keep/change/add verdicts) lives in [`evidence/dogfood-customer-360.md`](./evidence/dogfood-customer-360.md); most execution-verified rules below were probed there or in session **ws-697490-4**.

### inspect — Map/List only
`rest/InspectStateMachine.java:47-51` returns the value only if it is a `Map` or `List`; any scalar leaf yields `404 "Not found"` by design.

### Functions table — all execution-verified
`f:defaultValue` (present→source / null→fallback), `f:now` (epoch ms + local timestamp), `f:parseDate` (`yyyy-MM-dd; ms` → epoch ms), `f:uuid`, `f:text` (int→string), `f:includes`, `f:concat`, `f:listOfMap` (parallel arrays under a model path → list of maps), `f:removeKey` (drops a key from each map in a list).

### Coerce-the-null-flag — selector asymmetry
`:boolean(null=true)` writes `true` when the source is null but writes **nothing** when present (not `false`) — verified, session ws-697490-4. Hence the same-mapper `f:defaultValue(..., boolean(false))` coercion.

### COMPUTE — multi-line works
In `GraphJs.compute` (`skills/GraphJs.java:180-195`) the statement text reaches `context.eval(JS, text)` with its newlines intact; the `command.replace('\n',' ')` flatten in `processCommands` (`GraphJs.java:161-165`) runs *after* `compute()` on a separate copy, so it never touches a COMPUTE expression.

### `{path}` substitution rule (`GraphLambdaFunction.substituteVarIfAny`/`replaceWithParameter`, lines 208-261)
Substitution scans the right-hand side for `{…}` groups (`Utility.extractSegments`/`findEndBracket`, `Utility.java:1316-1356`) and resolves each group's inner text as a state-machine path **only when that text contains no colon, newline, tab, or carriage return** (the guard at `GraphLambdaFunction.java:238`); otherwise the group is assumed to be a JS function body or JSON object and left verbatim. Scanning is **innermost-first** (`extractSegments` walks right-to-left via `lastIndexOf`), so a plain `{model.x}` resolves wherever it sits — inline, or nested inside an object literal or IIFE body — while an object literal such as `{tag: {model.x}}` keeps its outer braces (their inner text holds a `:`) as a JS object and the inner `{model.x}` still resolves.

### RESET + tight-loop guard
`RESET` → `GraphMath.resetNodes` removes the named nodes from the `nodeSeen` set. The tight-loop guard is `GraphExecutor`/`GraphTraveler.checkFrequency`, which aborts with `400 "executed too frequently"` when a node exceeds `graph.node.high.frequency` (default 10) hits within `graph.max.loop.interval` (default 1000ms).

### Statement verbs — verification status
`IF/THEN/ELSE`, `COMPUTE`, inline `MAPPING:`, `NEXT`, `DELAY`, and `EXECUTE` are **execution-verified**. `RESET` and `BEGIN/END` are **source-verified** (read from `GraphMath` / `GraphLambdaFunction`): `RESET` clears nodes for loops; `BEGIN/END` bound which statements iterate under `for_each` (and `for_each` itself is execution-verified).

### graph.extension — target resolution
The target loads from config `location.graph.deployed`, default `classpath:/graph/{id}.json`, **read-only** — a different store from `export`/`import`, which use `location.graph.temp` (default `file:/tmp/graph`). A `classpath:` location requires a rebuild/restart; the Companion API cannot deploy at runtime.
