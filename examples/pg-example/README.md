# reactive-postgres-client example

This is a worked example to demonstrate the database access patterns for the reactive-postgres library.

Before using this example, please build the maven project at root and the reactive-postgres library if you
have not published the libraries in the artifactory of your organization.

## Running this example application

Before you run this example application, please ensure that you have a PostGreSQL server running and update
...
To test if this application is running, visit "http://127.0.0.1:8100/health"

You will see a health check page like this:

```json
{
  "dependency": [
    {
      "route": "postgres.health",
      "status_code": 200,
      "service": "postgres",
      "href": "127.0.0.1",
      "message": "Read (15 ms), Write (5.591 ms)",
      "required": true
    }
  ],
  "origin": "2026010549cd93b4539b4d64a10cbbbe1cf66582",
  "name": "pg-example",
  "status": "UP"
}
```
...
The ReactivePgConfig class in the library is configured to scan your organization package path
...
For usage pattern, please review the TempTestData class and TempRepo interface in the "src/test/models" and
"src/test/repository" folders.
...
PostGreSQL prepared statement supports two ways of parameter binding
