...
New phase specs should mirror the shape of the existing four ([requirements-gathering.md](./requirements-gathering.md), [graph-design.md](./graph-design.md), [build.md](./build.md), [test.md](./test.md)): Purpose → Boundary → Inputs → Output artifact + schema → Operating protocol → Step checklist → Gate → Mock/placeholder rule → Completion rule. The existing specs are the template by example — this is contributor guidance, not part of running the workflow.
