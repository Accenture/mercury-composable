---
title: HTTP Response Streaming
summary: Stream token segments and progress events over plain HTTP - Server-Sent Events
  or chunked transfer - by sending events to the caller's reply route until end of
  transmission.
layer: operate
audience: [developer]
keywords: [streaming, sse, server-sent events, tokens, chunked, ndjson, x-event-stream]
---

# HTTP Response Streaming

*Guide: How a function streams an HTTP response progressively - token by token, event by
event.*
...
This marker is internal protocol between the callee and the HTTP edge - like
`x-stream-id` and `x-ttl`, it never appears on the wire. The HTTP client sees only
standard HTTP: chunked transfer encoding, and Server-Sent Events framing when the content
...
public class TokenProducer implements TypedLambdaFunction<EventEnvelope, Void> {
    ...
    public Void handleEvent(Map<String, String> headers, EventEnvelope request, int instance) {
        ...
        out.write("tokens", Map.of("n", 2));         // a named (typed) SSE event
        out.close(Map.of("usage", 42));              // end of transmission
        return null;
    }
}
```
...
| `first(status, contentType, ttlSeconds)` | as above, plus an idle-allowance override between segments |
| `write(segment)` | one `data` segment - String, byte[] or Map |
| `write(eventName, segment)` | a named segment - the name maps to the SSE `event:` field |
| `close()` / `close(metadata)` | end of transmission - metadata rides the terminal SSE `done` event |
| `fail(exception)` | in-band failure - rendered as an SSE `error` event, or truncation in chunked mode |
...
- End of transmission is the terminal `event: done`, carrying the `close(...)` metadata.
...
**Chunked mode** (any other content type): String and byte[] segments append verbatim;
...
- The endpoint `timeout` acts as an *idle* allowance - each arriving segment extends it.
  A producer may override the idle allowance with `first(status, contentType, ttlSeconds)`.
...
- A client that stops reading is bounded by a 1 MB server-side buffer; beyond that the
...
The lambda-example application ships a runnable demo: the `hello.sse` function serves
...
```shell
node examples/lambda-example/scripts/sse-client.mjs
```
...
unchanged. `x-event-stream` is the idiom for progressive text/JSON delivery - tokens and
events - with SSE framing and in-band terminal signals. If both headers appear on a
response, `x-event-stream` wins and the stray `x-stream-id` is ignored with a warning.
