...
| `schema.registry.properties` | `String` (comma-sep paths) | `file:/tmp/config/schema-registry.properties, classpath:/schema-registry.properties` | Registry client template location(s); file path falls back to classpath. Entries pass verbatim to the Confluent client — [authentication](kafka-flow-adapter.md#schema-auth) (`bearer.auth.*` OAuth 2.0, basic auth) and SSL. OAuth token endpoint URLs found in a template are auto-registered on the JVM `org.apache.kafka.sasl.oauthbearer.allowed.urls` allow-list. |
...
| `secondary.kafka.producer.properties` / `secondary.kafka.consumer.properties` | `String` (comma-sep paths) | `file:/tmp/config/secondary-kafka-*.properties, classpath:/secondary-kafka-*.properties` | [twin-kafka](twin-kafka.md): secondary-cluster client template location(s); same mechanics as the primary templates. |
