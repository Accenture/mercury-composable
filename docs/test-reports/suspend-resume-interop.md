...
deploy, and the `helpers/redis-standalone` server.
...
| Rust app | `examples/minigraph-playground` from mercury main (post PR #186 merge), port 8100 (its default has since been synced to **8085**, same as Java, so manual engine-swap tests reuse one browser URL — for side-by-side runs, override one app's `rest.server.port`) |
...
| Store functions | `v1.redis.persist.model` (SETEX) / `v1.redis.retrieve.model` (GETDEL) — the Java extension module and its Rust crate twin |
