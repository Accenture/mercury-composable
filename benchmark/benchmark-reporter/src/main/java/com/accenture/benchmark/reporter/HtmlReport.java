...
public final class HtmlReport {
    ...
    public static String render(Map<String, String> env, List<WorkloadResult> results) {
        ...
        for (WorkloadResult r : results) {
            ...
            chip(h, "ok", num(s.count(), 0));
            chip(h, "failures", num(r.failures(), 0));
            ...
            h.append("<div class=\"chart\"><h3>Latency distribution</h3>\n")
                    .append(svgHistogram(s, color)).append("</div>\n");
            h.append("<div class=\"chart\"><h3>Latency by percentile</h3>\n")
                    .append(svgPercentiles(s, color)).append("</div>\n");
            h.append("</div>\n</div>\n");
        }
    ...
    private static String svgPercentiles(Stats s, String color) {
        ...
        for (int i = 0; i < vals.length; i++) {
            double x = left + i * step;
            double y = top + plotH - plotH * (vals[i] / yMax);
            g.append(String.format(Locale.US, "<circle cx=\"%.2f\" cy=\"%.2f\" r=\"3.5\" fill=\"%s\"/>\n",
                    x, y, color));
            g.append(String.format(Locale.US,
        ...
        g.append(String.format(Locale.US, "<text x=\"%.2f\" y=\"%.2f\" class=\"axis\">latency ms</text>\n",
                left - 48, top + plotH / 2));
        g.append("</svg>");
        return g.toString();
    }

    private static String svgOpen(int w, int ht) {
    ...
    private static String num(long v, int decimals) {
