...
- Field pipeline runs Snyk then SonarQube; both stages PASSED and the build DEPLOYED
  same-day (2026-09-04). The anticipated Overall-Code Sonar surprise round did not
  materialize — Eric's best-effort local Sonar scans held up. Full arc: field Snyk
  rejection → upstream remediation → field deployment, within one day, with no new
  release cut (the quick-patch path).
