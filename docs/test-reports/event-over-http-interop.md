...
3. **The flagship drive: an unchanged engine flow executed a Python function.** The
   composable-example's shipped `event-over-http-declarative` Event Script flow — the
   [zero-code demo](../guides/event-over-http.md#zero-code-demo) of this report's
   earlier rounds — ran against the Python demo host in the lambda-example's slot
   (same port 8085, same public `hello.declarative` route), with **zero engine change
   and zero engine configuration change**. The reply carried `"language": "python"`;
   the flow's business correlation id arrived injected as the read-only
   `my_correlation_id` header (the `my_cid` tag contract); `x-flow-id` and the flow's
   `x-ttl` were delivered; and the Python host's log line showed the **engine's trace
   id** in the engine-consistent log format — presentation parity extending to the
   wrapper family (playbook items 3 and 5).
4. **The one engine change shipped separately, in lock-step.** `graph.task`'s
   ...
   (Java [PR #292](https://github.com/Accenture/mercury-composable/pull/292), Rust
   [PR #212](https://github.com/Accenture/mercury/pull/212)), released in **v4.11.11**
   — deployed knowledge graphs can name polyglot routes directly.
