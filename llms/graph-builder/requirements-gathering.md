...
The required output is a design-ready graph brief, structured per the [Design-Ready Brief Template](#design-ready-brief-template) (the canonical schema below) — purpose, invocation/output/state contracts, source catalog, mappings, control flow, failure behavior, non-functionals, tests, scope boundary, decisions, open questions with blocker classification, and the gate result. The brief may live in one markdown file or be split into companion files when the feature is large.
...
See [briefs/hello-world-requirements-brief.md](briefs/hello-world-requirements-brief.md) for a filled design-ready brief using the canonical schema (its `/design` and `/build` counterparts live alongside it in `briefs/`).