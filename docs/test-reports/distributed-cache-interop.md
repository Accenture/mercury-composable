...
module in v4.12.9, the Rust port merged as mercury PR #285.* The same report is kept in the Rust repository's `docs/test-reports/`.
...
> `Timeout for 5000 ms` (re-probed 2026-09-20 01:17 UTC); the Java Layer 2 and 3 rows are unchanged.
...
assertion depends neither on a Redis outage nor on which of two timeouts fires first. Both fixes ship with this report — the Java one in the same change as this page, the Rust one in the companion change in the mercury repository (its twin of this page).
...
cause's message. The status is now resolved from the **cause chain** (`Utility.getStatusFromException`:
the first `AppException`, `TimeoutException` → 408 or `IllegalArgumentException` → 400 wins; 500 only
when none is present), one rule shared by both mappers and by `EventStreamWriter.fail`, with the
documented table in the Event Envelope reference updated to match. Re-probed live after the fix with
Redis stopped (2026-09-20 01:17 UTC): the Java Layer 1 GET, POST and DELETE answer **408
`Timeout for 5000 ms`** — the code the Rust engine already returned — while Layers 2 and 3 keep
answering 500 `Command timed out after 5 second(s)`, because that is Lettuce's own timeout exception,
which carries no status. The fix ships in the mercury-composable change that carries this amendment
(the follow-up to PR #426).
