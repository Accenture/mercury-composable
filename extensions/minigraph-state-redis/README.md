...
and they register automatically**; point the graph's `suspend`/`resume` nodes at them
through the `task` property:
...
The same keys as the sync-over-async extension, so an application configures Redis once
...
Any composable function honoring the same put/get contract can replace this module —
