...
- conv-thread-id-names-the-thing — A thread id names the THING, never its kind — and an existing thread is never renamed (Eri… — overdue (sslu 21 > archive_window 20) at the v4.12.17 seam; its rule shipped upstream as agent-memory v4.41.1 (DECAY.md §1), so the fact carried only history — 2026-Q3.md
- conv-schema-example-not-a-fact — `continuity.md`'s header shows the metadata footer as an EXAMPLE, and that example's id mu… — overdue (sslu 21 > archive_window 20) at the v4.12.17 seam; the parser fix shipped upstream as agent-memory v4.41.2, the header placeholder stays as the honest example — 2026-Q3.md
...
- l1-caller-checks-reply-status — A function that awaits an RPC must check the reply's STATUS before reading its body — the… — faded past archive_window 20 at the docs/deploy-without-rebuild seam; both rules shipped in v4.12.13 (PRs #426, #427) and live in the code and event-envelope-reference.md — 2026-Q3.md
- exception-status-from-cause-chain — A function's error status comes from its CAUSE CHAIN — the first `AppException` (its statu… — faded past archive_window 20 at the docs/deploy-without-rebuild seam; both rules shipped in v4.12.13 (PRs #426, #427) and live in the code and event-envelope-reference.md — 2026-Q3.md
