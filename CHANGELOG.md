...
1. **Generic exception context for `exception=` handlers — one handler can serve every
   ...
   `error.stack` (when the failure carries one) in the state machine — the same
   `error.code/message/stack` vocabulary as Event Script's flow exception handlers, plus
   `source` — so one island-anchored handler node serves the whole graph without naming
   any failing node in its data mapping. `graph.api.fetcher`, `graph.task` (including
   `task=async.http.request`), `graph.extension` and the suspend/resume store calls all
...
2. **The orchestrator pattern: a parent graph delegates independently resumable subgraph
   ...
   `unit-test-orchestrator` / `unit-test-sub-suspend`. (ADR-0013)
...
3. **Dynamic variables now resolve in every statement command — completing the generic
   ...
   `DELAY:` values in `graph.math` / `graph.js` statements resolve `{namespace.key}`
   references at execution time, so a generic handler can retry *whichever node routed to
   it*: `RESET: {error.source}, error-handler` + `NEXT: {error.source}` (and pace with a
...
1. **`graph.task` input mapping now supports `model.{key}` staging targets, restoring Event
   Script parity for dynamic variables.** An `input[]` entry whose RHS starts with `model.`
   stages a variable in the graph's state machine — instead of silently landing in the request
   body under a literal `model` key — so a later entry can reference it as a dynamic variable,
   e.g. `input.body.person_id -> model.person_id` followed by
   `text(/api/mdm/profile/{model.person_id}) -> url` (the same idiom as Event Script's
   `text(Bearer {model.token}) -> headers.Authorization`). This matches the `graph.api.fetcher`
   and `graph.extension` input mappings, which already supported it. Engine-managed model
   metadata remains immutable: the CompileGraph gate and the playground pre-run check reject a
   reserved target at validation time, and the new runtime path calls the same shared guard,
   covering single-node execution of drafts.
...
2. **tutorial-13 now invokes the platform's AsyncHttpClient (`async.http.request`) instead of a
   throwaway demo function.** The `graph.task` tutorial becomes a worked example of an HTTP
   ...
   deployment compile and at `instantiate graph` for a dry-run) while the authored and exported
...
1. **Suspension is a destination: edge-mode checkpoints and jump-mode decisions replace the
   `suspend=true` property (ADR-0012).** A suspension point is now declared by **how a node
   reaches the reserved `suspend` node**, discriminated purely by graph shape. **Edge mode** —
   a working node with a drawn edge to `suspend` redirects there when its skill completes; the
   edge is the declaration, a continuation edge is mandatory (compiler-enforced, shape-only —
   no statement inspection), and a resumed run continues along it without re-executing the
   node: byte-for-byte the prior suspensible behavior, so **every valid earlier model deploys
   and behaves identically unmodified** (the prior gate already required the checkpoint edge).
   **Jump mode** — a `graph.math` decision returns `suspend` from its IF-THEN-ELSE and is
   **re-executed against the new request input on every resume**, so it re-decides each time:
   ...
   written by earlier releases resume correctly. The workflow-suspension guide, tutorial-14
   help, `graph.suspend` skill help and the AI grammar (`minigraph-commands.json`) are
   rewritten as one story; covered by engine tests for both modes, the deprecation-compat
   shapes, the new gate rules and a jump-mode wait loop across consecutive suspensions.

2. **tutorial-14's manager approval step is now a real decision with three outcomes.** The
...
3. OSS update: spring boot 4.0.5 and 3.5.13, log4j2 2.25.4, tomcat 10.1.54 and 11.0.21
...
Support of default value in an input parameter of a data dictionary item.
...
Support OffsetDateTime (database data type TIMESTAMPTZ) in SimpleMapper
...
3. "trace.log.header" added to application.properties - when tracing is enabled, this inserts the trace-ID of the 
   transaction in the log context. For more details, please refer to the [Developer Guide](https://accenture.github.io/mercury-composable/guides/CHAPTER-5/)
