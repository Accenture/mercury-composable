...
@SimplePlugin
public class ParseDate implements PluginFunction {
    ...
    public Object calculate(Object... input) {
        if (input.length == 2 && input[1] instanceof String text) {
            ...
            Instant instant = LocalDate.parse(value, dateFormatter).atStartOfDay(ZoneId.systemDefault()).toInstant();
            ...
            throw new IllegalArgumentException("Syntax error. Syntax: text(pattern, iso | local | ms)." +
                    "e.g. f:parseDate(input.body.date, text(MM/dd/yyyy, ms))");
        }
    }
}
